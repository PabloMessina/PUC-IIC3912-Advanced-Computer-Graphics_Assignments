...
{
	float4 worldPosition = mul(modelMatrix, float4(input.inPosition, 1));
	float4 worldNormal = mul(modelMatrix, float4(input.inNormal, 0));
...
float3 BRDF(float3 normal, float3 lightDirection, float3 viewDirection, float3 halfVector, float3 diffuse)
{
	...
	float coef = round(intensity * intensityFactor) / intensityFactor;
	return coef * (lambertBRDF(normal, lightDirection, diffuse) + blinnPhongBRDF(normal, halfVector, specularColor, shininess));
}
...
float4 FShaderLine(fragmentAttributes input) : SV_Target
{
